...
public class KitchenCookAdapter extends BaseAdapter {
    private Context context;
    private int layout;
    private List<CartItems> cartItemsList;
    private String sIDDonHang;

    public KitchenCookAdapter(Context context, int layout, List<CartItems> cartItemsList, String sIDDonHang) {
        ...
        viewHolder.cartItemsCheck.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(viewHolder.cartItemsCheck.isChecked()){
                    ...
                    databaseReference.child("Cook").child(sIDDonHang).child("cartItemsList").child(cartItemsList.indexOf(cartItems) + "").child("check").setValue(true);
                }
                else if(!viewHolder.cartItemsCheck.isChecked()){
                    cartItems.setCheck(false);
                    DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference();
                    databaseReference.child("Cook").child(sIDDonHang).child("cartItemsList").child(cartItemsList.indexOf(cartItems) + "").child("check").setValue(false);
                }
            }
        });
